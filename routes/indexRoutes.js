...
router.post("/message", async (req, res) => {
  ...
  try {
    if (!thread) {
      const agents = await AgentModel.find({}).sort({
...
router.post("/resolve/:threadId", async (req, res) => {
  ...
  try {
    ...
    if (findThread.isResolved === false) {
...
router.get("/get-thread/:agentId", async (req, res) => {
  ...
  try {
    const findAgent = await ThreadModel.find({
      isResolved: false,
      agent: agentId,
    }).populate("messages");
    findAgent.sort((a, b) => {
...
router.get("/get-thread", async (req, res) => {
  try {
    const findAgent = await ThreadModel.find({ idResolved: false }).populate(
      "messages"
    );
